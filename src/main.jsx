...
function Overview({ items, go }) {
  ...
  return <><PageTitle eyebrow="Decision cockpit · Q3 planning" title="Make the next best bet." desc="A single view of customer signal, delivery reality and measurable outcomes." action={<button className="primary" onClick={() => go('Opportunity backlog')}><Plus size={14} /> Review opportunities</button>} />
    ...
    <div className="grid-two"><section className="card pulse"><div className="card-head"><div><h2>Portfolio pulse</h2><p>Share of this sample by customer outcome</p></div><button className="link" onClick={() => go('Prioritisation matrix')}>View matrix <ChevronRight size={15} /></button></div>{[['Growth',42,'#5c50d7'],['Retention',28,'#9b8afb'],['Revenue',18,'#f1a66a'],['Risk / trust',12,'#70c6bb']].map(([label, value, color]) => <div className="bar-row" key={label}><span>{label}</span><div className="bar"><i style={{ width: `${value}%`, background: color }} /></div><b>{value}%</b></div>)}<p className="definition">Percentages show the illustrative share of opportunities tagged to each outcome.</p></section>
      <section className="card signal"><div className="card-head"><div><h2>Signal to action</h2><p>Sample inputs and what they inform</p></div><span className="badge mint">3 signals</span></div>{[['01','Jira Product Discovery','Teams need a faster first win','18 votes · 6 interviews'],['02','Miro','Onboarding friction cluster','12 notes · workshop 14'],['03','Excel / Sheets','Q3 capacity updated','72% allocated']].map(x => <div className="signal-row" key={x[0]}><span className="num">{x[0]}</span><div><b>{x[1]} <em>{x[2]}</em></b><small>{x[3]}</small></div><ChevronRight size={16} /></div>)}</section></div>
    ...
    <section className="card explanation"><h2>Scenario, objective and guardrails</h2><div className="explanation-grid"><div><b>Illustrative scenario</b><p>A fictional B2B product team is planning Q3 with more customer requests than delivery capacity.</p></div><div><b>Objective</b><p>Choose a small set of bets that improve activation, protect enterprise trust and create measurable learning.</p></div><div><b>Goals</b><p>Reduce time-to-first-value, validate revenue signals and reserve capacity for risk work.</p></div><div><b>Stakeholders</b><p>Customers, engineering, design, sales, support and leadership need a transparent trade-off.</p></div></div><div className="risk-note"><ShieldAlert size={17} /><span><b>Risks and consequences:</b> scoring can create false precision; prioritising growth alone could delay enterprise safeguards. I show assumptions and keep a 20% risk allocation visible.</span></div></section></>;
}
...
function Backlog({ items, allItems, onAdd }) {
  ...
  return <><PageTitle eyebrow="Input layer · Jira Product Discovery" title="Opportunity backlog" desc="Turn customer evidence into opportunities the whole team can understand." action={<button className="primary" onClick={() => setShowForm(!showForm)}>{showForm ? <X size={14} /> : <Plus size={14} />} {showForm ? 'Close form' : 'New opportunity'}</button>} />
    {showForm && <form className="card add-form" onSubmit={submit}><label>Opportunity title<input required value={draft.title} onChange={e => setDraft({ ...draft, title: e.target.value })} placeholder="e.g. Faster workspace setup" /></label><label>Team<input value={draft.team} onChange={e => setDraft({ ...draft, team: e.target.value })} /></label><label>Outcome<select value={draft.type} onChange={e => setDraft({ ...draft, type: e.target.value })}><option>Growth</option><option>Retention</option><option>Revenue</option><option>Risk</option></select></label><button className="primary" type="submit">Add illustrative opportunity</button></form>}
...
function Scoring({ items, update }) {
  ...
  return <><PageTitle eyebrow="Decision layer · editable model" title="Scoring workspace" desc="Edit assumptions, switch methods and see rankings recalculate immediately." />
